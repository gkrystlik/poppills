...
function showBiggerPicture(){
	...
	$popIn.addClass('is-visible').find('div').html($image.clone());
}

function hideBiggerPicture(){
...
function updatePrice(){
	...
	if(total > 0) { 
		$button.addClass('is-visible animate')
	...
	else { $button.removeClass('is-visible'); }
...
function addEventListeners(){
	...
	$('.js-showBiggerPicture').on('click', showBiggerPicture);
	$('.js-hideBiggerPicture').on('click', hideBiggerPicture);
	$('.js-updatePrice').on('change', updatePrice);
	$('.js-switchCurrency').on('click', switchCurrency);
	$('.js-submit').on('click', onSubmit);
	$('.js-order').on('click', order);
}
